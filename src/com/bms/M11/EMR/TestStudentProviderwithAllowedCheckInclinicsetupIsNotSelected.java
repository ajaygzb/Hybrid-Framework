...
public class TestStudentProviderwithAllowedCheckInclinicsetupIsNotSelected extends TestSetUp {
	// TC-43 DO NOT Allow Student to finalize Visit notes
	// TC-44 Allow Student to finalize Non Visit notes
	// TC-45 Allow Student to finalize Non Visit-DISCHARGE notes
	public String strActualtext = null;

	@Test(enabled = true, priority = 1)
	public void DONOTAlowStudentToFinalizeVisitNotes() throws InterruptedException {
		test = extent.startTest(
				"DO NOT Allow Student to finalize Visit notes with allow Student Note Sign in clinic setup :  Un-CHECKED",
				"DO NOT Allow Student to finalize Visit notes  with  allow Student Note Sign in clinic setup :  Un-CHECKED"
	...
	public void AllowStudentToFINALIZENOnVisitNotes() throws InterruptedException {
		test = extent.startTest(
				"Allow Student to finalize NonVisit  notes with  allow Student Note Sign in clinic setup :  Un-CHECKED",
				"Allow Student to finalize NonVisit  notes with  allow student Note Sign in clinic setup :  Un-CHECKED"
						+ "*****Current Browser******" + CurrentBrowserName + "*****Browser Version******"
						+ caps.getVersion());
		test.assignCategory("Regression", "On" + "  " + CurrentBrowserName,
		...
		try {
			...
			HandlingCalendars.datepick(driver, CreateNotePage.StartofCareDate(driver), "14/06/2015");// Enter
																										...
																										// CreateNotePage.DateofService(driver),
																										// "14/06/2015");
	...
	public void AllowStudentToFinalizNonVisitDischargenotes() throws InterruptedException {
		test = extent.startTest(
				"Allow Student to finalize Non visit discharge  notes with  allow Student Note Sign in clinic setup :  Un-CHECKED",
				"Allow Student to finalize Non visit discharge  notes with  allow Student Note Sign in clinic setup :  Un-CHECKED"
						+ "*****Current Browser******" + CurrentBrowserName + "*****Browser Version******"
						+ caps.getVersion());
		test.assignCategory("Regression", "On" + "  " + CurrentBrowserName,
