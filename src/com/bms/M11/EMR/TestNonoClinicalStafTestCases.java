...
public class TestNonoClinicalStafTestCases extends TestSetUp {
	public String strActualtext = null;
	public List<WebElement> actual = null;

	// TC-31 DO NOT Allow Non-Clinical Staff to finalize Visit notes
	// TC-32 Allow Non-Clinical Staff to finalize Non Visit notes
	// TC-33 Allow Non-Clinical Staff to finalize Non Visit-DISCHARGE notes
	@Test(enabled = true, priority = 1)
	public void DONOTAlowNonClinicalStafToFinalizeVisitNotes() throws InterruptedException {
		test = extent.startTest(
				"DO NOT Allow Non clinical staff to finalize Visit notes with Don't allow Non-Clinical Staff Note Sign on insurance class:  CHECKED",
				"DO NOT Allow Non clinical staff to finalize Visit notes  with Don't allow Non-Clinical Staff Note Sign on insurance class:  CHECKED"
		...
		try {
			...
			Commons.waitforElement(driver, By.xpath("//div[contains(@ng-bind-html,'vmNote.providerMessage')]"), 30);
			Assert.assertEquals(
					driver.findElement(By.xpath("//div[contains(@ng-bind-html,'vmNote.providerMessage')]")).getText(),
	...
	public void AllowNonclinicalstaffToFinalizeNonVisitnotes() throws InterruptedException {
		test = extent.startTest(
				"Allow Non clinical staff to finalize NonVisit  notes with Don't allow Non-Clinical Staff Note Sign on insurance class:  CHECKED",
				"Allow Non clinical staff to finalize NonVisit  notes with Don't allow Non-Clinical Staff Note Sign on insurance class:  CHECKED"
						+ "*****Current Browser******" + CurrentBrowserName + "*****Browser Version******"
						+ caps.getVersion());
		test.assignCategory("Regression", "On" + "  " + CurrentBrowserName,
		...
		try {
			...
			driver.findElement(By.xpath("//textarea[@name='nonVisitStatement']"))
					.sendKeys("Non clinical staff can finalize non visit notes");
	...
	public void AllowNonclinicalstaffToFinalizNonVisitDischargenotes() throws InterruptedException {
		test = extent.startTest(
				"Allow Non clinical staff to finalize Non Visit Discharge notes with  with Don't allow Non-Clinical Staff Note Sign on insurance class:  CHECKED",
				"Allow Non clinical staff to finalize Non Visit Discharge  notes with Don't allow Non-Clinical Staff Note Sign on insurance class:  CHECKED"
						+ "*****Current Browser******" + CurrentBrowserName + "*****Browser Version******"
						+ caps.getVersion());
		test.assignCategory("Regression", "On" + "  " + CurrentBrowserName,
				this.getClass().getPackage().getName().toString());
