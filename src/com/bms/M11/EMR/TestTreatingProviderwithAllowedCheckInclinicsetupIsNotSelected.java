...
public class TestTreatingProviderwithAllowedCheckInclinicsetupIsNotSelected extends TestSetUp {
	public String strActualtext = null;

	// TC-46 DO NOT Allow Treating Provider to finalize Visit notes
	// TC-47 Allow Treating Provider to finalize Non Visit notes
	// TC-48 Allow Treating Provider to finalize Non Visit-DISCHARGE notes
	@Test(enabled = true, priority = 1)
	public void DONOTAlowTreatingProvideToFinalizeVisitNotes() throws InterruptedException {
		test = extent.startTest(
				"DO NOT Allow Treating provider to finalize Visit notes with allow Treating provide Note Sign in clinic setup :  Un-CHECKED",
				"DO NOT Allow Treating provider to finalize Visit notes  with  allow Treating provide Note Sign in clinic setup :  Un-CHECKED"
	...
	public void AllowTreatingProviderToFINALIZENOnVisitNotes() throws InterruptedException {
		test = extent.startTest(
				"Allow Treating provider to finalize NonVisit  notes with  allow Treating provide Note Sign in clinic setup :  Un-CHECKED",
				"Allow Treating provider to finalize NonVisit  notes with  allow Treating provide Note Sign in clinic setup :  Un-CHECKED"
						+ "*****Current Browser******" + CurrentBrowserName + "*****Browser Version******"
						+ caps.getVersion());
		test.assignCategory("Regression", "On" + "  " + CurrentBrowserName,
	...
	public void AllowTreatingProviderToFinalizNonVisitDischargenotes() throws InterruptedException {
		test = extent.startTest(
				"Allow Treating provider to finalize Non visit discharge  notes with  allow Treating provide Note Sign in clinic setup :  Un-CHECKED",
				"Allow Treating provider to finalize Non visit discharge  notes with  allow Treating provide Note Sign in clinic setup :  Un-CHECKED"
						+ "*****Current Browser******" + CurrentBrowserName + "*****Browser Version******"
						+ caps.getVersion());
		test.assignCategory("Regression", "On" + "  " + CurrentBrowserName,
