...
public class TestStudentProviderInInsuranceClassTestCases extends TestSetUp {
	// TC-25 DO NOT Allow Non-Clinical Staff to finalize Visit notes
	// TC-26 Allow Non-Clinical Staff to finalize Non Visit notes
	// TC-27 Allow Non-Clinical Staff to finalize Non Visit-DISCHARGE notes
	public String strActualtext = null;

	@Test(enabled = true, priority = 1)
	public void DontAllowStudentToFinalizeVisitNotes() throws InterruptedException {
		test = extent.startTest(
				"Dont Allow Student To Finalize Visit Notes   with Don't allow Student Note Sign on insurance class:  unChecked",
				"Dont Allow Student To Finalize Visit Notes  with  Don't allow Student Note Sign on insurance class:  unChecked"
		...
		try {
			...
			Assert.assertTrue(Commons.waitforElement(driver,
					By.xpath("//table//td[contains(text(),'You must select a co-signer.')]"), 90));
			...
			System.out.println("DO NOT Allow Student to  finalize Visit notes is successful");
			test.log(LogStatus.INFO, "DO NOT Allow student to finalize Visit notes is successful");
			test.log(LogStatus.INFO, "Finalized Note.");
			Commons.screenshot();
		} catch (Exception e) {
			System.out.println("DO NOT Allow Student to  finalize Visit notes is not successful ");
			Assert.assertFalse(true,
					"DO NOT Allow student to  finalize Visit notes fail" + Throwables.getStackTraceAsString(e));
		} catch (AssertionError e) {
			Assert.assertFalse(true, Throwables.getStackTraceAsString(e));
		}
	}

	@Test(enabled = true, dependsOnMethods = { "DontAllowStudentToFinalizeVisitNotes" }, priority = 2)
	public void AllowStudentProvidertofinalizeNonVisitnotes() throws InterruptedException {
		test = extent.startTest(
				"Allow Treating Provider to finalize NonVisit  notes  with Don't allow Student Note Sign on insurance class:  unChecked",
				"Allow Treating Provider to finalize NonVisit  notes  with Don't allow Student Note Sign on insurance class:  unChecked"
						+ "*****Current Browser******" + CurrentBrowserName + "*****Browser Version******"
						+ caps.getVersion());
		test.assignCategory("Regression", "On" + "  " + CurrentBrowserName,
	...
	@Test(enabled = true, dependsOnMethods = { "DontAllowStudentToFinalizeVisitNotes" }, priority = 3)
	public void AllowStudentProvidertofinalizNonVisitDischargenotes() throws InterruptedException {
		test = extent.startTest(
				"Allow Student Provider to finalize Non Visit Discharge notes  with Don't allow Student Note Sign on insurance class:  unChecked",
				"Allow Treating Student to finalize Non Visit Discharge  notes with Don't allow Student Note Sign on insurance class:  unChecked"
						+ "*****Current Browser******" + CurrentBrowserName + "*****Browser Version******"
						+ caps.getVersion());
		test.assignCategory("Regression", "On" + "  " + CurrentBrowserName,
				this.getClass().getPackage().getName().toString());
		...
		try {
			...
			Commons.waitForLoad(driver);
			Commons.existsElement(driver, By.xpath("//button[@value='Save Note']"));
			Commons.ExistandDisplayElement(driver, By.xpath("//a[@href='#Assessment']"), 80);
			System.out.println("Successfully redirected to APage");
			Commons.Explicitwait();
			Commons.waitForLoad(driver);
			APageUtils.FinalizeNote(driver);
